var Zepto = function() {
    ...
    return T.matches = function(t, e) {
            ...
            if (!e) return T.Z();
            if ("string" == typeof e)
                if (e = e.trim(), "<" == e[0] && l.test(e)) r = T.fragment(e, RegExp.$1, i),
                    e = null;
                else {
...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto),
    ...
    function(t) {
        ...
        function j(e) {
            e.processData && e.data && "string" != t.type(e.data) && (e.data = t.param(e.data, e.traditional)), !e.data || e.type && "GET" != e.type.toUpperCase() || (e.url = E(e.url, e.data),
                e.data = void 0);
        }
    ...
    function() {
        ...
        else if ("undefined" != typeof webkitAudioContext) ctx = new webkitAudioContext();
        ...
        HowlerGlobal.prototype = {
            volume: function(vol) {
                ...
                if (vol = parseFloat(vol), vol && vol >= 0 && 1 >= vol) {
                    ...
                    for (var key in self._howls)
                        if (self._howls.hasOwnProperty(key) && self._howls[key]._webAudio === !1)
                            for (var i = 0; i < self._howls[key]._audioNode.length; i++) self._howls[key]._audioNode[i].volume = self._howls[key]._volume * self._volume;
                    return self;
                }
            ...
            _setMuted: function(muted) {
                ...
                for (var key in self._howls)
                    if (self._howls.hasOwnProperty(key) && self._howls[key]._webAudio === !1)
                        for (var i = 0; i < self._howls[key]._audioNode.length; i++) self._howls[key]._audioNode[i].muted = muted;
            }
        };
        ...
        if (Howl.prototype = {
                load: function() {
                    ...
                    for (var i = 0; i < self._urls.length; i++) {
                        var ext, urlItem;
                        if (self._format) ext = self._format;
                        else {
                    ...
                    if (self._src = url, self._webAudio) loadBuffer(self, url);
                    else {
                ...
                play: function(sprite, callback) {
                    ...
                    return "function" == typeof sprite && (callback = sprite), sprite && "function" != typeof sprite || (sprite = "_default"),
                        self._loaded ? self._sprite[sprite] ? (self._inactiveNode(function(node) {
                            node._sprite = sprite;
                            var timerId, pos = node._pos > 0 ? node._pos : self._sprite[sprite][0] / 1e3,
                                duration = self._sprite[sprite][1] / 1e3 - node._pos,
                                loop = !(!self._loop && !self._sprite[sprite][2]),
                                soundId = "string" == typeof callback ? callback : Math.round(Date.now() * Math.random()) + "";
                            if (function() {
                                    var data = {
                                        id: soundId,
                                        sprite: sprite,
                                ...
                                if (4 !== node.readyState) return self._clearEndTimer(timerId),
                                    function() {
                                        var sound = self,
                                            playSprite = sprite,
                                            fn = callback,
                ...
                pos: function(pos, id) {
                    ...
                    for (var i = 0; i < self._audioNode.length; i++)
                        if (self._audioNode[i].paused && 4 === self._audioNode[i].readyState) return self._webAudio ? self._audioNode[i]._pos : self._audioNode[i].currentTime;
                },
                pos3d: function(x, y, z, id) {
                    var self = this;
                    if (y = "undefined" != typeof y && y ? y : 0, z = "undefined" != typeof z && z ? z : -.5, !self._loaded) return self.on("play", function() {
                        self.pos3d(x, y, z, id);
                    }), self;
                ...
                fade: function(from, to, len, callback, id) {
                    ...
                    for (var i = 1; steps >= i; i++) ! function() {
                        var change = self._volume + ("up" === dir ? .01 : -.01) * i,
                ...
                _nodeById: function(id) {
                    for (var self = this, node = self._audioNode[0], i = 0; i < self._audioNode.length; i++)
                ...
                _activeNode: function() {
                    for (var self = this, node = null, i = 0; i < self._audioNode.length; i++)
                ...
                on: function(event, fn) {
                    ...
                    if ("function" == typeof fn) events.push(fn);
                    else
                        for (var i = 0; i < events.length; i++) fn ? events[i].call(self, fn) : events[i].call(self);
                    return self;
                },
                off: function(event, fn) {
                    for (var self = this, events = self["_on" + event], fnString = fn.toString(), i = 0; i < events.length; i++)
                ...
                if (url in cache) obj._duration = cache[url].duration, loadSound(obj);
                else {
    ...
    function() {
        var notes = {
                "1C": new Howl({
                    urls: ["./medias/261-C.mp3"]
                }),
                "1Cs": new Howl({
                    urls: ["./medias/277-C-sharp.mp3"]
                }),
                "1D": new Howl({
                    urls: ["./medias/293-D.mp3"]
                }),
                "1Ds": new Howl({
                    urls: ["./medias/311-D-sharp.mp3"]
                }),
                "1E": new Howl({
                    urls: ["./medias/329-E.mp3"]
                }),
                "1F": new Howl({
                    urls: ["./medias/349-F.mp3"]
                }),
                "1Fs": new Howl({
                    urls: ["./medias/369F-sharp.mp3"]
                }),
                "1G": new Howl({
                    urls: ["./medias/391-G.mp3"]
                }),
                "1Gs": new Howl({
                    urls: ["./medias/415-G-sharp.mp3"]
                }),
                "2A": new Howl({
                    urls: ["./medias/440-A.mp3"]
                }),
                "2As": new Howl({
                    urls: ["./medias/466-A-sharp.mp3"]
                }),
                "2B": new Howl({
                    urls: ["./medias/495-B.mp3"]
                }),
                "2C": new Howl({
                    urls: ["./medias/523-C.mp3"]
                }),
                "2Cs": new Howl({
                    urls: ["./medias/545-C-sharp.mp3"]
                }),
                "2D": new Howl({
                    urls: ["./medias/587-D.mp3"]
                }),
                "2Ds": new Howl({
                    urls: ["./medias/622-D-sharp.mp3"]
                }),
                "2E": new Howl({
                    urls: ["./medias/659-E.mp3"]
                }),
                "2F": new Howl({
                    urls: ["./medias/698-F.mp3"]
                }),
                "2Fs": new Howl({
                    urls: ["./medias/698-F-sharp.mp3"]
                }),
                "2G": new Howl({
                    urls: ["./medias/783-G.mp3"]
                }),
                "2Gs": new Howl({
                    urls: ["./medias/830-G-sharp.mp3"]
                }),
                "3A": new Howl({
                    urls: ["./medias/880-A.mp3"]
                }),
                "3As": new Howl({
                    urls: ["./medias/932-A-sharp.mp3"]
                }),
                "3B": new Howl({
                    urls: ["./medias/987-B.mp3"]
                })
            },
            lockEvent = {};
